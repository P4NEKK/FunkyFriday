...
local UI = urlLoad("https://raw.githubusercontent.com/P4NEKK/FunkyFriday/main/Library/Library.lua")
local themeManager = urlLoad("https://raw.githubusercontent.com/P4NEKK/FunkyFriday/main/ThemeManager.lua")

local metadata = urlLoad("https://raw.githubusercontent.com/P4NEKK/FunkyFriday/main/metadata.lua")
...
local Window = UI:CreateWindow({
    Title = string.format('ZazaHB - Funky Friday | updated: %s', metadata.updated),
    AutoShow = true,
...
Groups.Misc = Tabs.Main:AddRightGroupbox('Fun')
...
Groups.Credits = Tabs.Miscellaneous:AddRightGroupbox('Credits')
    ...
    addRichText(Groups.Credits:AddLabel('<font color="#3da5ff">zaza#1337</font> - developer'))
    addRichText(Groups.Credits:AddLabel('<font color="#de6cff">pro_tcz</font> - helper'))
      Groups.Credits:AddButton('Copy discord', function()
        if pcall(setclipboard, "https://discord.gg/QGzGfc3Ra4") then
            UI:Notify('Successfully copied discord link!', 5)
        end
    end)


Groups.Misc = Tabs.Miscellaneous:AddRightGroupbox('Other')

    Groups.Misc:AddButton('Unload script', function() pcall(shared._unload) end)

    Groups.Misc:AddLabel('Menu Bind'):AddKeyPicker('MenuToggle', { Default = 'Delete', NoUI = true })
